...
void merge(vector<int>& a, int lo, int mid, int hi) {
    ...
    peakBufBytes = max(peakBufBytes, (long long)(tmp.size() * sizeof(int)));
...
int main() {
    ...
    ofstream outfile("mergesort.csv");
    printf("N,time_ops,mem_bytes\n");
    outfile << "N,time_ops,mem_bytes\n";
    for (int N = 1000; N <= 256000; N *= 2) {            // DOUBLE N each row
        ...
        outfile << N << "," << timeOps << "," << peakBufBytes << endl;
    }
    outfile.close();
    return 0;
}
