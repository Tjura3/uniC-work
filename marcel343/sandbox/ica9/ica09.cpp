...
vector<long> dijkstra(const WGraph& g, int src) {
    ...
    //return {};
    vector<long> dist(g.V, LONG_MAX); //distance or cost
    vector<bool> settled(g.V, false); //final spot
    priority_queue<pair<long,int>, vector<pair<long,int>>, greater<>> pq; 
    //pair is the values we are storing,
    //vector is the underlying container we are storing it in,
    //and greater means if the parent is greater than the child, they swap. like a heap.
    
    //also note with the way this is defined, we are defining the slot in the vector to be the value that contains the distance I think.
    dist[src] = 0; //source is zero
    pq.push({0, src}); //distance, key
    while(!pq.empty()){
        int u = pq.top().second; pq.pop(); //u is now the integer or location of dist
        if(settled[u]) continue; //continue if we already stuck it down
        settled[u] = true; //now whatever distance is in there is staying that way.
        for(pair<int,int> e : g.adj[u]){ //for each pair of value, distance in the graph
            if(!settled[e.first] && dist[u] + e.second < dist[e.first]){ //we first check if its settled or not, 
                //then if the distance we currently are at is smaller than what it currently is
                dist[e.first] = dist[u] + e.second;     //if it is, we change it to the new closest distance
                pq.push({dist[e.first], e.first});      //and we push that change to the pq
            }
        }
    }
    //finally returning the completed list of distances.
    return dist;
}
