...
struct Node {
    int   key[3];                       // key[0..n-1] sorted; slot 2 only during overflow  [1 | 2]
...
void addKey(Node* x, int k, Node* rc) {
    ...
    //addkey is a function to throw a number inside the already existing node.
    //initially tried iterating through it from 0, but resizing ment more calculations because you need to move everything down afterwards and it was a mess.
    //realized its better to just go in reverse, n-1 because it will either be 1 or 2 and keys located at 0 and 1.
    int i = x->n-1; 
    while(i >= 0 && x->key[i] > k){
        x->key[i + 1] = x->key[i];
...
Node* split(Node* x, int& mid) {
    ...
    //this makese sense to me thankfully
    mid = x->key[1];
    Node* right = new Node(x->key[2]);

    right->child[0] = x->child[2];
    right->child[1] = x->child[3];
