...
long fibMemo(int n, vector<long>& memo) {
    ...
    if(n <= 1) return n; //base case, would be either 1 or 0.
...
int rodCut(const vector<int>& price, int n) {
    ...
    //im just gonna assume that n is the length, and cutting it at n-x = length
    //overall, you make a vector that holds the best value if the rod was best[index] length
    //i is the rod at that length,
    //j is finding the max it can be when cut up, from cutting it at all angles.
    //the best is only stored when it maximizes the value
    //therefore, when best[n] is called, it is returning the maximized price at that value.
    vector<int> best(n + 1, 0); //one index, but also 0 needs to be 0.

    for(int i = 1; i <= n; i++){
        int mval = 0;
        for(int j = 1; j <= i; j++){
            mval = max(mval, price[j] + best[i-j]);
        }
        best[i] = mval;
    }
    return best[n];
