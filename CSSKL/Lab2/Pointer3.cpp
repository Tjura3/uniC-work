...
int* dynamicAlloc3(int size) {
	...
	for (int i = 0; i < size; ++i) {
		...
		*(a + i) = i; //a[i] = 10
        //how about "*(a++) = 10;"?
        //no because *a would be pointing past the end of the array, you can fix it by doing a - (whatever amount traversed) but thats code
        //that is so bad, your parents would disown you.
	}
	for (int i = 0; i < size; ++i) {
...
int main()
{
	int a[] = { 1,2,3,4,5 }; //its not new, so this is coded on the stack.
	int b[5] = { 1,2,3,4,5 };
	int c[10] = { 1,2,3,4,5 };
